...
class Keyboard:
    ...
    def typing(self, typing_input):
        ...
        for input in typing_input:
            output.append(self.press(input))
        return "".join(output)
...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        return "Accountholder: {}, Deposits: {}, Withdraws: {}".format(self.holder, self.deposit_time, self.withdraw_time)
...
def insert(link, value, index):
    ...
    curr_idx, curr = 0, link
    while curr is not Link.empty:
        if index==curr_idx: 
            curr.rest = Link(curr.first, curr.rest)
            curr.first = value
            return
        curr_idx += 1
        curr = curr.rest
...
def prune_small(t, n):
    ...
    """
    t.branches = sorted(t.branches, key=lambda branch:branch.label)[:n]
