...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        for card in player.deck.cards:
            card.attack, card.defense = card.attack+opponent_card.attack, card.defense+opponent_card.defense
        delete_idx = []
        for i, card in enumerate(opponent.deck.cards):
            if card.attack==opponent_card.attack or card.defense==opponent_card.defense:
                delete_idx.append(i)
        opponent.deck.cards = [card for i, card in enumerate(opponent.deck.cards) if i not in delete_idx]
