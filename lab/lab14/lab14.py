...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        min_v = min([branch.label for branch in t.branches])
        t.branches = list(filter(lambda x:x.label==min_v, t.branches))
...
def num_splits(s, d):
    ...
    12
    """
    def run(list_a, list_b, remain):
        if not remain:
            return 1 if abs(sum(list_a)-sum(list_b))<=d else 0
        else:
            return run(list_a+[remain[0]], list_b, remain[1:]) + run(list_a, list_b+[remain[0]], remain[1:])
...
class Account:
    ...
    def __str__(self):
        "*** YOUR CODE HERE ***"
        return "{}'s Balance: ${}".format(self.holder, self.balance)

    def __repr__(self):
        "*** YOUR CODE HERE ***"
        deposit_time = sum([a[0]=='deposit' for a in self.transactions])
        withdraw_time = sum([a[0]=='withdraw' for a in self.transactions])
        return "Accountholder: {}, Deposits: {}, Withdraws: {}".format(self.holder, deposit_time, withdraw_time)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.deposited or self.holder!=check.holder:
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldr(lst, lambda v,l: Link(first=v, rest=l) if pred(v) else l, Link.empty)
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda l,v: Link(first=v, rest=l), Link.empty)
