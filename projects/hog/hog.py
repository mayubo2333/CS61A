...
def piggy_points(score):
    ...
    "*** YOUR CODE HERE ***"
    value = score**2
    digit_num = len(str(value))-1
    v = 9
    while digit_num>=0:
        if value>=10**digit_num:
            digit = value//10**digit_num
            value -= (10**digit_num)*digit
            v = min(digit, v)
            digit_num -= 1
        else:
            v = 0
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls:
        ...
        scores = piggy_points(opponent_score)
    return scores
    # END PROBLEM 3
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_v, opponent_v = str(player_score), str(opponent_score)
    if len(opponent_v)==1:
        return False
    elif len(player_v)==1:
        return int(player_v)<int(opponent_v[1])
    else:
        return int(player_v[0])<int(opponent_v[0]) and int(player_v[1])<int(opponent_v[1])
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    "*** YOUR CODE HERE ***"
    def update_score(score0, score1, player_score, who):
        if not who:
            score0 = player_score
        else:
            score1 = player_score 
    ...
    while score0<goal and score1<goal:
        ...
        num_roll = strategy(player_score, opponent_score)
        player_score += take_turn(num_roll, opponent_score, dice, goal)
        score0, score1 = update_score(score0, score1, player_score, who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    We'll learn about how to work around this in a future lecture, but it is not required for this problem.
    To fix this, you have two options:
    1) Rather than reassigning [var] to its new value, 
        create a new variable to hold that new value. Use that new variable in future calculations.
    ...
    "*** YOUR CODE HERE ***"
    def say(score0, score1):
        gain = (score0 - last_score) if who == 0 else (score1 - last_score)
        if gain > running_high:
            print("Player {} has reached a new maximum point gain. {} point(s)!".format(who, gain))
            running_high_new = gain
        else:
            running_high_new = running_high
        score = score0 if who == 0 else score1
        return announce_highest(who, score, running_high_new)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    run = make_averaged(roll_dice, trials_count)
    max_value = 0
    arg_max = -1
    for i in range(1, 11):
        value = run(i, dice)
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if more_boar(score+piggy_points(opponent_score), opponent_score):
        return 0
    elif piggy_points(opponent_score) >= cutoff:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
