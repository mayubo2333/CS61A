...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    return lambda p: any([s in topic for s in remove_punctuation(p).lower().split()])
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    correct_num = 0
    for type_w, ref_w in zip(typed_words, reference_words):
        if type_w==ref_w:
            correct_num += 1
            
    return (correct_num/len(typed_words))*100.0 if len(typed_words) else 0.0
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        output, min_diff = None, float("inf") 
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    s = [[0 for _ in range(len(goal))] for _ in range(len(start))]
    for i in range(len(start)):
        for j in range(len(goal)):
            if i==0 and j==0:
                s[i][j] = int(start[0]!=goal[0])
            elif i==0:
                s[i][j] = j + int(start[0] not in goal[:(j+1)])
            elif j==0:
                 s[i][j] = i + int(goal[0] not in start[:(i+1)])
            else:
                if start[i]==goal[j]:
                    s[i][j] = s[i-1][j-1]
                else:
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    time = list()
    for timestamp in times_per_player:
        time_this_player = list()
        for i in range(1, len(timestamp)):
            time_this_player.append(timestamp[i]-timestamp[i-1])
        time.append(time_this_player)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    res = [list() for _ in player_indices]
    
    for word_idx in word_indices:
        time_list = [time(match, player_idx, word_idx) for player_idx in player_indices]
        fastest_player_this_word, fastest_time_this_word = -1, float("inf")
        for player_idx in player_indices:
            if time_list[player_idx] < fastest_time_this_word:
                fastest_time_this_word = time_list[player_idx]
                fastest_player_this_word = player_idx
        res[fastest_player_this_word].append(word_at(match, word_idx))
