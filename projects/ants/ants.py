...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if place.ant.can_contain(self) and not self.can_contain(place.ant):
                place.ant.store_ant(self)
            elif self.can_contain(place.ant) and not place.ant.can_contain(self):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not curr.is_hive:
            if curr.bees and self.min_range<=dist<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        reflected_amount = amount if amount<self.health else amount+self.damage
        for bee in self.place.bees[:]:
            bee.reduce_health(reflected_amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown==0:
            ...
            self.chew_countdown = max(0, self.chew_countdown-1)
...
# END Problem 12
    ...
    def __init__(self, gamestate=None):
        if gamestate:
            ScubaThrower.construct(gamestate)
        self.health = 1
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        curr = self.place.exit
        while curr is not None:
            ant = curr.ant
            if ant and not ant.has_doubled:
                if hasattr(ant, 'damage'):
                    ant.damage = 2*ant.damage
                    ant.has_doubled = True
            if ant and ant.is_container and ant.ant_contained and not ant.ant_contained.has_doubled:
                ant.ant_contained.damage = 2*ant.ant_contained.damage  
                ant.ant_contained.has_doubled = True
            curr = curr.exit
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slow_countdown>0 and gamestate.time%2==1:
            destination = None
        elif self.scare_countdown>0:
            destination = None if self.place.entrance.is_hive else self.place.entrance
            self.scare_countdown = max(self.scare_countdown-1, 0)
        self.slow_countdown = max(self.slow_countdown-1, 0)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        insects_and_distances = dict()
        for bee in self.place.bees:
            insects_and_distances[bee] = 0
        curr, dist = self.place.entrance, 1
        
        while not curr.is_hive:
            if curr.ant:
                insects_and_distances[curr.ant] = dist
                if curr.ant and curr.ant.is_container and curr.ant.ant_contained:
                    insects_and_distances[curr.ant.ant_contained] = dist
            for bee in curr.bees:
                insects_and_distances[bee] = dist 
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(self.base_damage - 0.25*distance - self.insects_shot/16, 0)
        # END Problem Optional 2
