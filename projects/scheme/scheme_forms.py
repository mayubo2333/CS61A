    ...
    if scheme_symbolp(signature):
        ...
        while curr is not nil:
            if not scheme_symbolp(curr.first):
                raise SchemeError('non-symbol: {0}'.format(curr.first))
            curr = curr.rest
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            val = test if scheme_nullp(clause.rest) else eval_all(clause.rest, env)
            return val
...
def make_let_frame(bindings, env):
    ...
    while curr is not nil:
        item = curr.first
        validate_form(item, min=2, max=2)
        names = Pair(item.first, names)
        validate_formals(names)
        
        value = scheme_eval(item.rest.first, env)
...
def do_define_macro(expressions, env):
    ...
    try:
        signature = expressions.first.first 
        assert(scheme_symbolp(signature))
    ...
    env.define(signature, procedure)
    return signature
