...
def balanced(m):
    ...
    if is_mobile(end(left(m))):
        flag = flag and balanced(end(left(m)))
    if is_mobile(end(right(m))):
        flag = flag and balanced(end(right(m)))
    return flag
...
def totals_tree(m):
    ...
    left_one, right_one = end(left(m)), end(right(m))
    left_branch = totals_tree(left_one) if is_mobile(left_one) else tree(size(left_one))
    right_branch = totals_tree(right_one) if is_mobile(right_one) else tree(size(right_one))
    branches = [left_branch, right_branch]
    
    return tree(label, branches=branches)
...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    True
    """
    r = thors_replacement if label(t)=="thor" and is_leaf(t) else label(t)
    branch_list = [replace_thor_at_leaf(branch, thors_replacement) for branch in branches(t)]
    return tree(label=r, branches=branch_list)
...
def has_path(t, word):
    ...
    if rest:
        return label(t)==first and any([has_path(branch, rest) for branch in branches(t)])
    else:
